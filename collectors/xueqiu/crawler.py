...
class XueqiuCrawler(BaseCrawler):
    ...
    def _parse_xueqiu_time(self, time_str: str) -> datetime:
        """
        解析雪球的时间格式
    ...
    def _is_in_time_window(self, time_str: str) -> bool:
        """判断时间是否在过去指定小时内"""
        try:
            parsed_time = self._parse_xueqiu_time(time_str)
    ...
    def _search_keyword(self, page, keyword: str) -> List[Dict]:
        ...
        try:
            # 雪球搜索URL
            search_url = f"{self.base_url}/query?q={keyword}"
            ...
            if "登录" in page_content and "注册" in page_content:
                self.logger.warning(f"[{keyword}] 雪球可能需要登录才能获取完整搜索结果")

            # 等待搜索结果加载
            try:
                # 雪球搜索结果通常在 .article-item 或 .status-item 中
                page.wait_for_selector('.article-item, .status-item, .search-item', timeout=15000)
                self.logger.info(f"[{keyword}] 搜索结果列表已加载")
            except:
                self.logger.warning(f"[{keyword}] 等待列表超时")

            # 截图调试
            page.screenshot(path=f"output/logs/xueqiu_{keyword}_search.png")
            ...
            try:
                ...
                for item in list_items[:self.max_items_per_keyword]:
                    try:
                        # 提取标题
                        title = ''
                        for title_selector in ['.title', 'h3', 'h4', '.article-title', '.status-title', 'a']:
                            try:
                        ...
                        # 提取摘要
                        summary = ''
                        for summary_selector in ['.summary', '.brief', '.desc', '.content', '.article-content', 'p']:
                            try:
    ...
    def fetch(self) -> List[NewsItem]:
        ...
        self.logger.info(f"开始采集雪球 - 关键词: {self.keywords}")
